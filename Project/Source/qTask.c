...
void qTaskGetInfo(qTask * task, qTaskInfo * info)
{
	...
	info->stackFree = 0;                      //堆栈空余大小
	stackEnd = task->stackBase;
	while((*stackEnd++ == 0) && (stackEnd <= task->stackBase + task->stackSize / sizeof(qTaskStack)))
	{
		info->stackFree++;
	}
	info->stackFree *= sizeof(qTaskStack);
